...
async function main() {
    ...
    program.command('mint-poatekers <path> <chunk-number> <offset>')
        .description('mints all poatekers')
        .action(deployPoatekers)


    program.command('deploy')
        .description('deploy an instance of the PoatekersERC721 NFT contract')
        .option('-o, --output <deploy-file-path>', 'Path to write deployment info to', config.deploymentConfigFile || 'minty-deployment.json')
        .option('-n, --name <name>', 'The name of the token contract', 'PoatekNFTs')
        .option('-s, --symbol <symbol>', 'A short symbol for the tokens in this contract', 'PTKS')
        .action(deploy)
...
async function deployPoatekers(path, chunkNumber, offset) {
    let files = await readdir(path);

    if(!path) {
        console.log('Please inform the path!');
        return;
    }
    ...
    if (offset === undefined) {
        ...
        console.log('Offset cannot be larger than 20');
        return;
    }
    ...
    const chunkSize = 200;
    console.log('Chunk size:' + chunkSize + ' Chunk:' + chunkNumber + ' Offset:' + offset);

    var chunks = [];
    for (let i = 0; i < files.length; i += chunkSize) {
        const chunk = files.slice(i, i + offset + chunkSize);
        chunks.push(chunk);
    }

    if (chunkNumber >= chunks.length) {
        console.log(`chunkNumber must be lower than the max number of chunks (${chunks.length})`);
        return;
    }

    console.log(chunks[chunkNumber]);

    for (let file of chunks[chunkNumber]) { 
        var filePath = path + '/' + file;
        console.log('Creating NFT (' + filePath + ')')
        await createNFT(filePath, {
            name: file.split('.png')[0],
            description: 'poateker'
        });			
    }
}
